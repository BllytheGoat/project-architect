...
import { continueInterview } from "@/lib/services/interview";
...
/** GET /api/projects/:id/interview/next — next question or "complete". */
export async function GET(_req: Request, ctx: Ctx) {
  try {
    const { projectId } = await ctx.params;
    await loadOwnedProject(projectId);
    const turn = await continueInterview(projectId);
